...
static char* icon_dir = 0;
static char* icon_name = 0;
...
int tray_init(struct tray *tray) {
  ...
  icon_dir = dirname(strdup(tray->icon));
  icon_name = basename(strdup(tray->icon));
  strip_ext(icon_name);
...
void tray_update(struct tray *tray) {
  app_indicator_set_icon_full(indicator, icon_name, "Ideckia icon");
